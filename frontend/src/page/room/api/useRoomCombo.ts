...
export const getRoomCombo = async ({
    ...
    >
> => {
    return requestApi('get', ROOM_COMBO_API, { houseId, signal });
};
...
export const useRoomCombo = (houseId?: string, config?: UseRoomCombo) => {
    return useQuery<ExtractFnReturnType<QueryFnType>>({
        queryKey: ['COMBO_ROOM', houseId],
        queryFn: ({ signal }) => getRoomCombo({ houseId, signal }),
        ...config,
    });
};
